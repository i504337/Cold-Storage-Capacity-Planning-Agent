...
from milestones import (  # noqa: F401  (re-exported for backward compat)
    log_m1_achieved, log_m1_missed,
    log_m2_achieved, log_m2_missed,
    log_m3_achieved, log_m3_missed,
    log_m4_achieved, log_m4_missed,
    log_m5_achieved, log_m5_missed,
)
...
def get_system_prompt() -> str:
    base_prompt = """You are a cold storage capacity planning assistant for temperature-controlled EWM warehouse storage types: cryo freezers (−80°C), ultra-cold refrigerators (−20°C), and controlled cold rooms (2–8°C).\n\nYou help warehouse planners and sourcing specialists by:\n- Answering questions about current and projected storage availability\n- Computing rolling 1–2 week time-phased capacity projections per storage type\n- Identifying capacity breach risks for incoming orders\n- Mapping materials to their target storage types automatically from EWM master data\n\nIMPORTANT: You MUST use tools to retrieve live data. Never fabricate, guess, or invent data. Relay tool errors verbatim without adding suggestions.\n\nWhen calling tools that support pagination, always set the page size parameter (top, limit, pageSize, etc.) to a maximum of 100 items to prevent context overflow. Inform the user when this limit is applied.\n\nIf a query is ambiguous (storage type unclear, date range not specified, material unknown), ask a clarifying question before retrieving data.\n\nAlways clearly state when projections are estimates based on open order data — they are not confirmed physical facts. Always include the data freshness context in your responses.\n\nIf a tool returns an error, report the error message exactly as received. Do not suggest workarounds or alternative approaches unless asked.""" + _DEFENSIVE_PROMPT_SUFFIX
...
class SampleAgent:
    ...
    async def _run_agent(
        ...
        """Core agent execution logic with business step instrumentation.

        Extracted from stream() to allow OpenTelemetry span context managers
        without triggering GeneratorExit errors inside async generators.
        ...
        with tracer.start_as_current_span("m4_query_answered"):
            ...
            try:
                ...
                logger.info(
                    "M4.achieved: Query answered — response_latency_ms: %d",
                    elapsed_ms,
                )
                return response
            except Exception as exc:
                logger.warning(
                    "M4.missed: Query could not be answered — reason: api_failure. Error: %s",
                    exc,
                )
                raise
